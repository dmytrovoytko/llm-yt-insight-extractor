...
SUBTOPICS_PROMPT_TEMPLATE = """You are an expert content analyst. Extract 1-5 key subtopics from the following transcript context.
...
ACTIONABLE_IDEAS_PROMPT_TEMPLATE = """You are an expert life coach. Extract exactly 5 top actionable ideas from the following transcript context.

For each idea, provide:
1. A concise, actionable title (max 7 words, must start with a verb if possible)
2. A clear description (max 3 sentences) explaining how to implement it
